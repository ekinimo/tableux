...
pub struct FormulaPool {
    ...
    pub vars: HashMap<String, VarIdx>,
    pub var_names: Vec<String>,
}

impl FormulaPool {
    pub fn display(&self, idx: FormulaIdx) -> String {
        match self[idx] {
    ...
    pub fn var(&mut self, t: impl Into<String>) -> FormulaIdx {
        let t = t.into();
    ...
    pub fn and(&mut self, t1: FormulaIdx, t2: FormulaIdx) -> FormulaIdx {
    ...
    pub fn or(&mut self, t1: FormulaIdx, t2: FormulaIdx) -> FormulaIdx {
        let formula = Formula::Or(t1, t2);
        let len = FormulaIdx(self.formulas.len());
        self.formulas.push(formula);
        len
    }
    pub fn implies(&mut self, t1: FormulaIdx, t2: FormulaIdx) -> FormulaIdx {
        let formula = Formula::Implies(t1, t2);
        let len = FormulaIdx(self.formulas.len());
        self.formulas.push(formula);
        len
    }
    pub fn not(&mut self, t1: FormulaIdx) -> FormulaIdx {
...
pub struct TableuxIdx(pub usize);

#[derive(Clone, Debug)]
pub struct TableuxElement {
    pub parent: Option<TableuxIdx>,
    pub sign: bool,
    pub formula: FormulaIdx,
}
...
#[derive(Clone, Debug, Default)]
pub struct Tableux {
    pub formulas: FormulaPool,
    pub elements: Vec<TableuxElement>,
    pub children: HashMap<TableuxIdx, (Option<TableuxIdx>, Option<TableuxIdx>)>,
    pub hasnt_processed: HashSet<TableuxIdx>,
    pub open: Vec<TableuxIdx>,
    pub closed_branches: Vec<(TableuxIdx, TableuxIdx)>,
    pub cant_progress: HashSet<TableuxIdx>,
}

impl Tableux {
    pub fn hypothesis(formulas: FormulaPool, formula: FormulaIdx) -> Self {
    ...
    pub fn get_parents(&self, idx: TableuxIdx) -> Vec<TableuxElement> {
        let mut ret = vec![];
        let mut idx = idx;
        loop {
            let elem = &self[idx];
    ...
    pub fn closes(&self, elem: &TableuxElement, path: &[TableuxElement]) -> bool {
        path.iter()
    ...
    pub fn proove(&mut self, mut n: usize) {
        loop {
    ...
    pub fn process_unprocessed_nodes(&mut self) {
        if self.open.is_empty() {
    ...
    pub fn get_leafs(&self, idx: TableuxIdx) -> Vec<TableuxIdx> {
        let mut ret = vec![];
...
pub fn parse_lisp(pool: &mut FormulaPool, p: pest::iterators::Pair<'_, Rule>) -> FormulaIdx {
    match p.as_rule() {
